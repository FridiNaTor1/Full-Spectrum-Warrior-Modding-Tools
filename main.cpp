...
std::vector<char> adjust_to_original_size(const std::vector<char> &original,
    ...
    if (modified.size() > original.size()) {
        ...
        // Trim trailing NUL bytes if available
...
bool patch_no_downs_limit(const fs::path &dll_path) {
...
int patch_gamespy_to_openspy(const fs::path &dll_path) {
...
void write_resolution(const fs::path &install_dir, int width, int height) {
...
int main(int argc, char **argv) {
    ...
    try {
        if (cmd == "patch-no-downs") {
            ...
            fs::path dll = argv[2];
            bool ok = patch_no_downs_limit(dll);
            if (!ok) {
                std::cerr << "Pattern not found in " << dll << " (maybe already patched?)\n";
                return 2;
            }
            std::cout << "No-downs limit patch applied to " << dll << "\n";
            ...
            fs::path dll = argv[2];
            int count = patch_gamespy_to_openspy(dll);
            if (count == 0) {
                std::cerr << "gamespy.com not found in " << dll
                          << " (maybe already patched?)\n";
                return 2;
            }
            std::cout << "Replaced " << count << " occurrence(s) in " << dll << "\n";
            ...
            int total_blocks = 0;
            auto paks = iter_pak_files(chapters);
            for (const auto &pak : paks) {
                total_blocks += patch_no_mission_failures_in_pak(pak);
            }
            if (paks.empty()) {
                std::cerr << "No .pak files found in " << chapters << "\n";
                return 2;
            }
            std::cout << "Patched " << total_blocks << " block(s) across " << paks.size()
                      << " PAK(s)\n";
            ...
            std::cout << "Patched " << total_changes << " ammo value(s) across "
                      << paks.size() << " PAK(s)\n";
            ...
            write_resolution(install, width, height);
            std::cout << "Resolution.cfg updated in " << install << "\n";
        } else {
            usage(argv[0]);
            return 1;
        }
